...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      try {
        const data = await checkAuthStatus() as AuthResponse;
        if (data && typeof data === "object" && "email" in data && "name" in data) {
          setUser({ email: data.email as string, name: data.name as string });
          setIsLoggedIn(true);
        }
      } catch (error) {
  ...
  const signup = async (name: string, email: string, password: string) => {
    const data = await signupUser(name, email, password) as AuthResponse;
    if (data && typeof data === "object" && "email" in data && "name" in data) {
      setUser({ email: data.email as string, name: data.name as string });
      setIsLoggedIn(true);
    }
